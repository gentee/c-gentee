...
//   p_heap->count--;
// ??? ѕочему то при последнем элементе иногда получаетс€  выход за границы  
...
uint STDCALL mem_init( void )
{
   ...
   for ( i = 0; i < ABC_COUNT; i++ )
   {
	 ...
	 #else
      _lower[i] = ( ubyte )tolower( ( ubyte )i ) ;
	 #endif
      _bin[i] = 0xFF;
      if ( i >= '0' && i <= '9' )
         _name[i] = 1;
...
void  STDCALL mem_move( pvoid dest, pvoid src, uint len )
{
   ...
   {
      ilen = len >> 2;
      // области пам€ти пересекаютс€ и надо копировать с конца
...
pvoid  STDCALL mem_zero( pvoid dest, uint len )
{
   ...
   len &= 0x3;
   while ( len-- ) {
      pubyte p_dest1 = ((pubyte)p_dest)+1;
      //*((pubyte)p_dest)++ = 0;
      //*( (pubyte) p_dest1 ) = 0; †
      *((pubyte)p_dest)=0;
   }
